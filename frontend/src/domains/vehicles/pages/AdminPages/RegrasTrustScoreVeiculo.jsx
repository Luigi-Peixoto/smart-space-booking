...
function RegrasTrustScoreVeiculo() {
  ...
  async function carregar() {
    try {
      const [
        ...
        resExigenciaConfig,
        resRestricaoConfig,
        resEventosDisponiveis,
        resNiveisDisponiveis,
        resRestricoesDisponiveis,
      ] = await Promise.all([
      ...
      const porEvento = {};
      resEventosDisponiveis.data.forEach(
        ({ chave, deltaPadrao, parametroPadrao }) => {
          const existente = resEventosConfig.data.find(
            (r) => r.chave === chave,
          );
          porEvento[chave] = existente
            ? {
                id: existente.id,
                delta: existente.valorPrincipal,
                parametro: existente.valorSecundario ?? "",
                usandoPadrao: false,
              }
            : {
                id: null,
                delta: deltaPadrao,
                parametro: parametroPadrao ?? "",
                usandoPadrao: true,
              };
        },
      );
      ...
      resNiveisDisponiveis.data.forEach(({ chave, scoreMinimoPadrao }) => {
        const existente = resExigenciaConfig.data.find(
          (r) => r.chave === chave,
        );
        porNivel[chave] = existente
          ? {
              id: existente.id,
              scoreMinimo: existente.valorPrincipal,
              usandoPadrao: false,
            }
      ...
      const porChave = {};
      resRestricoesDisponiveis.data.forEach(
        ({ chave, valorPrincipalPadrao, valorSecundarioPadrao }) => {
          const existente = resRestricaoConfig.data.find(
            (r) => r.chave === chave,
          );
          porChave[chave] = existente
            ? {
                id: existente.id,
                valorPrincipal: existente.valorPrincipal,
                scoreMinimoParaExceder: existente.valorSecundario,
                usandoPadrao: false,
              }
            : {
                id: null,
                valorPrincipal: valorPrincipalPadrao,
                scoreMinimoParaExceder: valorSecundarioPadrao,
                usandoPadrao: true,
              };
        },
      );
  ...
  async function salvar(evento) {
    ...
    if (
      linha.delta === "" ||
      linha.delta === null ||
      Number.isNaN(Number(linha.delta))
    ) {
    ...
    try {
      const payload = {
        ...
        valorSecundario:
          linha.parametro !== "" && linha.parametro !== null
            ? Number(linha.parametro)
            : null,
  ...
  async function restaurarPadrao(evento) {
    ...
    if (
      !window.confirm(
        "Remover a configuração salva e voltar ao valor padrão do sistema?",
      )
    )
      return;
    setSalvandoEvento(evento.chave);
  ...
  async function salvarExigencia(nivel) {
    ...
    if (
      linha.scoreMinimo === "" ||
      linha.scoreMinimo === null ||
      Number.isNaN(Number(linha.scoreMinimo))
    ) {
  ...
  async function salvarRestricao(restricao) {
    ...
    if (
      linha.scoreMinimoParaExceder === "" ||
      linha.scoreMinimoParaExceder === null ||
      Number.isNaN(Number(linha.scoreMinimoParaExceder))
    ) {
    ...
    return (
      <div className="admin-container">
        <main className="admin-main">
          <p>Carregando regras de TrustScore...</p>
        </main>
      </div>
    );
  }
    ...
    <div className="admin-container">
      <main className="admin-main">
        ...
        <h2
          ...
          style={{ fontSize: "1.15rem", marginTop: "0.5rem" }}
        >
          Penalidades por evento
        </h2>
        <p className="regras-subtitulo">
          Eventos do ciclo de vida da reserva que penalizam o TrustScore do
          usuário.
        </p>
                ...
                >
                  <div>
                    ...
                    <input
                      ...
                      value={linha.delta ?? ""}
                      onChange={(e) =>
                        set(evento.chave, "delta")(e.target.value)
                      }
                    />
                  </div>
                  <div>
                    ...
                    {evento.parametroPadrao !== null ? (
                      <input
                        ...
                        onChange={(e) =>
                          set(evento.chave, "parametro")(e.target.value)
                        }
                      />
                    ) : (
                      <input
        ...
        <h2
          className="page-title"
          style={{ fontSize: "1.15rem", marginTop: "2rem" }}
        >
          Exigência mínima para reservar
        </h2>
        <p className="regras-subtitulo">
          ...
          {niveisDisponiveis.map((nivel) => {
            ...
            return (
              <div key={nivel.chave} className="regra-card">
                <div className="regra-card-header">
                  <h3 className="regra-nome">
                    {humanizarChave(nivel.chave)} exigência
                  </h3>
                  {!linha.usandoPadrao && (
                    <span
                      className="material-icons action-icon"
                      title="Restaurar padrão"
                      onClick={() =>
                        !salvando && restaurarPadraoExigencia(nivel)
                      }
                    >
                      restart_alt
                    </span>
                ...
                >
                  <div>
                    <label className="regras-label">
                      TrustScore mínimo pra reservar
                    </label>
                    <input
                      type="number"
                      className="regras-input"
                      ...
                      onChange={(e) =>
                        setExigencia(nivel.chave, "scoreMinimo")(e.target.value)
                      }
                    />
                  </div>
                </div>
        ...
        <p className="regras-subtitulo">
          Cada restrição mede uma coisa diferente (veja a descrição do card). O
          "valor limite" é o número que dispara o bloqueio; o TrustScore mínimo
          é o que o usuário precisa ter pra ultrapassar esse limite.
        </p>
        <div className="regras-grid">
          {restricoesDisponiveis.map((restricao) => {
            ...
            return (
              <div key={restricao.chave} className="regra-card">
                <div className="regra-card-header">
                  <h3 className="regra-nome">
                    {humanizarChave(restricao.chave)}
                  </h3>
                  {!linha.usandoPadrao && (
                    <span
                      className="material-icons action-icon"
                      title="Restaurar padrão"
                      onClick={() =>
                        !salvando && restaurarPadraoRestricao(restricao.chave)
                      }
                    >
                      restart_alt
                    </span>
