...
function CheckoutReservaVeiculo({ onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!todasPreenchidas) {
      setErro({
        tipo: "geral",
        mensagem:
          "Envie as 3 fotos (externa, interna e placa) para confirmar o check-out.",
      });
      return;
    }
    ...
    try {
      const [resultado] = await Promise.allSettled([
        fazerCheckOut(id, user.id, arquivosOrdenados),
    ...
    <div className="admin-container">
      ...
      <main className="admin-main">
        <div className="page-header">
          <h2 className="page-title">Check-out do Veículo</h2>
          <p>
            Envie as 3 fotos exigidas para confirmar a devolução do veículo.
          </p>
        </div>
              ...
              {!previews[slot.chave] ? (
                <div
                  ...
                  onClick={() =>
                    !carregando &&
                    document
                      .getElementById(`input-foto-checkout-${slot.chave}`)
                      .click()
                  }
                >
...
export default CheckoutReservaVeiculo;
